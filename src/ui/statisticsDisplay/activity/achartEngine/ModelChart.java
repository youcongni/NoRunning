...
public class ModelChart extends AbstractDemoChart {
	...
	public View initView(double[] heartRate) {
		...
		// 80, 150, Color.BLACK, Color.BLACK);//
		...
		// renderer.setBackgroundColor(Color.rgb(220, 178, 167));//
		// 设置背景色RGB(221,198,194)
		// renderer.setShowLabels(true);
		//
		// renderer.setMargins(new int[] { 70, 70, 50, 50 });//
		// 设置图表的外边框(上/左/下/右)
		...
		// heartRateCopy[0]-10, heartRateCopy[heartRateCopy.length-1]+10 }); //
		// 设置拖动时X轴Y轴允许的最大值最小值.
		...
		values.add(heartRate);// 序列1中点的y坐标
		Arrays.sort(heartRateCopy);
		int[] colors = new int[] { Color.RED, Color.BLUE, Color.BLACK };// 每个序列的颜色设置
		PointStyle[] styles = new PointStyle[] { PointStyle.POINT,
				PointStyle.POINT, PointStyle.POINT };// 每个序列中点的形状设置
		// XYMultipleSeriesRenderer renderer = buildRenderer(colors, styles);//
		// 调用AbstractDemoChart中的方法设置
		...
		setChartSettings(renderer, "", "速度(m/s)", "心率(times/min)", 0, 10, 80,
				150, Color.BLACK, Color.BLACK);// 调用AbstractDemoChart中的方法设置图表的renderer属性.
	...
	public View initView(double[] heartRate, double[] speed) {
		...
		setChartSettings(renderer, "运动模型", "速度(m/s)", "心率(times/min)", 0,
				speed[speed.length - 1], heartRateCopy[0],
				heartRateCopy[heartRateCopy.length - 1], Color.BLACK,
				Color.BLACK);// 调用AbstractDemoChart中的方法设置图表的renderer属性.
